...
export class Sonarr extends Service {
  ...
  protected mapToIndexer(entry: SonarrEntry): Indexer {
    const indexer = new Indexer(
      ...
      entry.protocol,
      entry.fields.find(
        (field) => field.name == SonarrFieldName.categories
      )!.value,
      entry.fields.find(
        (field) => field.name == SonarrFieldName.minimumSeeders
      )!.value,
      entry.fields.find(
        (field) => field.name == SonarrFieldName.baseUrl
      )!.value,
      entry.fields.find((field) => field.name == SonarrFieldName.apiKey)?.value,
      entry.fields.find(
        (field) => field.name == SonarrFieldName.animeCategories
      )!.value
    );
  ...
  protected generateDefaultBody(indexer: JackettIndexer): SonarrEntry {
    ...
    const supportedAnimeCategories = this.animeCategories.filter((id) =>
      indexer.categories.includes(id)
    );

    this.indexerSpecificConfiguration(
      indexer,
      supportedCategories,
      supportedAnimeCategories
    );
    ...
    return {
      ...
      fields: [
        ...
        {
          name: SonarrFieldName.animeCategories,
          value: supportedAnimeCategories,
        },
        {
          name: SonarrFieldName.animeStandardFormatSearch,
          value: supportedAnimeCategories.length != 0,
        },
  ...
  protected shouldAdd(indexer: JackettIndexer) {
    return (
      ...
      indexer.categories.some((category) =>
        this.animeCategories.includes(category)
      )
    );
  }
  ...
  protected containsAllWantedCategories(
    ...
    const availableAnimeCategories = this.animeCategories.filter((id) =>
      indexer.categories.includes(id)
    );

    this.undoIndexerSpecificConfiguration(
      indexer,
      current.categories,
      current.animeCategories
    );

    return (
      arrayEquals(current.categories, availableCategories) &&
      arrayEquals(current.animeCategories, availableAnimeCategories)
    );
  }
}
