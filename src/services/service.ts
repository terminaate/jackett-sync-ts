...
export abstract class Service {
  ...
  protected beforeValidate(): boolean | Promise<boolean> {
    return true;
  }

  async validate(): Promise<AxiosResponse> {
    if (!(await this.beforeValidate())) {
      throw new Error(`[${this.serviceName}] did not validated successfully`);
    }
  ...
  async getIndexers(): Promise<void> {
    this.indexers = await this.handleIndexersRequest(
      this.apiRoutes.getIndexerUrl()
    );
  }
  ...
  async sync(jackettIndexers: JackettIndexer[]) {
    ...
    const updateMap = new Map(
      this.indexers.filter(notEmpty).map((i) => [i.id, i.appId])
    );

    const addAndUpdateRequests = [
      ...add.map((indexer) =>
        this.add(indexer)
          .then(this.handleResponse)
          .catch(this.handleError(indexer))
      ),
      ...update.map((indexer) => {
        const appId = updateMap.get(indexer.id);
        return this.update(appId, indexer)
          .then(this.handleResponse)
          .catch(this.handleError(indexer));
      }),
    ];

    await Promise.allSettled(addAndUpdateRequests);
  }
  ...
  private handleResponse = (response: AxiosResponse) => {
    ...
    switch (response.status) {
      case 201:
        console.log(`[${this.serviceName}] Added ${name} successfully!`);
        break;
      case 202:
        console.log(`[${this.serviceName}] Updated ${name} successfully!`);
        break;
      default:
        console.log(
          `[${this.serviceName}] Response for ${name} was unexpected:`,
          response.status
        );
    }
  };

  private handleError = (indexer: JackettIndexer) => (error: any) => {
    if (error?.response) {
      ...
      console.error(
        ...
        }] Something went wrong with ${data.name}, error: ${
          (axiosError.response?.data as any)[0]?.errorMessage
        }`
      );
    } else {
      console.error(
        `[${this.serviceName}] Unexpected error during request`,
        error
      );
    }
  };
  ...
  protected handleIndexersRequest(url: string): Promise<Indexer[]> {
    return axios
      .get(url)
      .then((response) => {
        return response.data.map((entry: Entry) =>
          this.mapToIndexerWithCatch(entry)
        );
      })
      .then((indexers) => {
        return indexers.filter(
          (indexer: Indexer | undefined) => indexer && indexer.id !== undefined
        );
      })
      .catch((error) => {
        if (error && error.response) {
          ...
          console.error(
            `[${this.serviceName}][${
              axiosError.response?.status
            }] Couldn't get indexes, error: ${JSON.stringify(
  ...
  private mapToIndexerWithCatch(entry: Entry): Indexer | undefined {
    try {
      ...
      console.warn(
        `[${this.serviceName}] Indexer ${entry.name} could not be parsed, skipping for check`
      );
    }
  }
    ...
    return axiosResponsePromise
      .then((response) => {
        if (response.status == 201) {
          console.log(
            `[${this.serviceName}] Added ${response.data.name} successfully!`
          );
        } else if (response.status == 202) {
          console.log(
            `[${this.serviceName}] Updated ${response.data.name} successfully!`
          );
        } else {
          console.log(
            `[${this.serviceName}] Request successful, but unknown responseStatus`,
            response.data.name
          );
        }
      })
  ...
  private checkDifferences(jackettIndexers: JackettIndexer[]): {
    add: JackettIndexer[];
    update: JackettIndexer[];
  } {
    const idList = jackettIndexers.map((el) => el.id);
    const serviceIdList = this.indexers
      .map((indexer) => indexer.id)
      .filter(notEmpty);

    const diff = idList.filter((id) => !serviceIdList.includes(id));
    const shouldBeAddedIndexers = diff
      .map((indexersId) => {
        const indexer = jackettIndexers.find(
          (indexer) => indexer.id == indexersId
        )!;
        ...
        {
          console.debug(
            `[${this.serviceName}] Skipping add for ${indexer.id}, since there were no matching categories.`
          );
        }
      })
      .filter(notEmpty);
    ...
    const shouldBeUpdatedIndexers = same
      .map((indexersId) => {
        const jacketIndexer = jackettIndexers.find(
          (indexer) => indexer.id == indexersId
        )!;
        const existingIndexer = this.indexers.find(
          (indexer) => indexer.id == indexersId
        )!;
        if (this.shouldUpdate(existingIndexer, jacketIndexer)) {
          ...
          console.debug(
            `[${this.serviceName}] Skipping update for ${existingIndexer.id}, since no updates were detected`
          );
        }
      })
      .filter(notEmpty);
    ...
    notInJackett.forEach((indexer) => {
      console.warn(
        `[${this.serviceName}] Found indexer ${indexer} which is not in Jackett, please remove manually`
      );
    });
  ...
  protected shouldAdd(indexer: JackettIndexer) {
    return (
      indexer.categories.some((category) =>
        this.categories.includes(category)
      ) || this.doesIndexerSpecificRuleApply(indexer)
    );
  }

  protected shouldUpdate(current: Indexer, indexer: JackettIndexer) {
    return (
      !current.compare(indexer) ||
      !this.containsAllWantedCategories(current, indexer)
    );
  }

  protected containsAllWantedCategories(
    current: Indexer,
    indexer: JackettIndexer
  ): boolean {
    const availableCategories = this.categories.filter((id) =>
  ...
  protected doesIndexerSpecificRuleApply(indexer: JackettIndexer): boolean {
    return (
      -1 !==
      Config.indexSpecificRules.findIndex((indexSpecificRule) => {
        if (
          indexSpecificRule.service === Services.ALL ||
          indexSpecificRule.service === this.serviceName
        ) {
  ...
  protected indexerSpecificConfiguration(
    ...
    Config.indexSpecificRules.forEach((indexSpecificRule) => {
      if (
        ...
        if (indexer.id === indexSpecificRule.indexerId) {
          if (
            indexSpecificRule.category != null &&
            !supportedCategories.includes(indexSpecificRule.category)
          ) {
          ...
          if (
            indexSpecificRule.animeCategory != null &&
            !animeSupportedCategories.includes(indexSpecificRule.animeCategory)
          ) {
      ...
      if (
        ...
        if (indexer.id === indexSpecificRule.indexerId) {
          if (
            ...
            supportedCategories.splice(
              supportedCategories.indexOf(indexSpecificRule.category),
              1
            );
          }
          if (
            ...
            animeSupportedCategories.splice(
              animeSupportedCategories.indexOf(indexSpecificRule.animeCategory),
              1
            );
          }
        }
